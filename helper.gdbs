...
define llexstate
  if $argc > 1
    ...
    printf "%-20s %d\n", "lastline", $ls->lastline

    ltoken $ls $ls->t "t"
    ltoken $ls $ls->lookahead "lookahead"
...
define ltoken
  if $argc != 3
    ...
    set $i = $t.token - FIRST_RESERVED

    printf "%-20s %s ", $key, luaX_token2str($ls, $t.token)

    if $t.token == TK_NAME || $t.token == TK_STRING
      printf "%s", getstr($t.seminfo.ts)
    end

    if $t.token == TK_NUMBER
      printf "%d", $t.seminfo.r
    end
